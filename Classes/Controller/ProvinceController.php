	...
	public function importAction($flushDB = false, Tx_Buechertransport_Command_ImportCommandController &$obj) {
		...
		if($flushDB)	{
			...
			if(count($obj->provinceRepository->getRemovedObjects()) > 0)	{
				t3lib_div::devLog('Import-Task: Truncating successful.' , 'buechertransport', -1); //, $files);
			}	else 	{
				t3lib_div::devLog('Import-Task: Truncating failed.' , 'buechertransport', 3); //, $files);				
			}
		}
		...
		foreach ($files as $key => $file) {
			...
			foreach ($csv as $ln => $line) {
				...
				if (!in_array($line['city'], $cities))	{
					...
					$city->setName($line['abbr']);
					$obj->cityRepository->update($city);
					t3lib_div::devLog('Import-Task: City ' . $line['city'] . ' updated.' , 'buechertransport', -1);
				}	else 	{
					$city = $obj->cityRepository->findByName($line['city']);
					t3lib_div::devLog('Import-Task: City ' . $line['city'] . ' found in Repository.' , 'buechertransport', -1);
				}

				if (!in_array($line['libr'], $libs) && $city instanceof Tx_Buechertransport_Domain_Model_City)	{
		...
		// $this->provinceRepository->remove($province);
		// $this->flashMessageContainer->add('Your data has been successfully imported.');
		// $this->redirect('list');
		return true;
	}
